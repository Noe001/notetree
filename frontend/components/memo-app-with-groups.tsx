...
// グループ関連コンポーネントのインポート
import { MemoCreateDialogWithGroup } from "@/components/memo/memo-create-dialog-with-group";
...
type MemoEditorProps = {
  ...
  onUpdateMemo?: (id: string, updates: Partial<Memo>) => Promise<Memo>;
};

const MemoEditor = ({ memo, onToggleSidebar, onDeleteMemo, onShareMemo, isRightSidebarOpen, onUpdateMemo }: MemoEditorProps) => {
...
export default function MemoAppWithGroups() {
  ...
  const { memos, loading, error, fetchMemos, updateMemo } = useMemos(user?.id, selectedGroupId);
