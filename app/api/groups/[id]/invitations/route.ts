...
export async function POST(req: NextRequest, { params }: any) {
  try {
    ...
    if (group.ownerId !== user.id) {
      return NextResponse.json({ error: 'Forbidden: Only group owner can invite members' }, { status: 403 });
    }
    ...
    if (existingInvitation) {
      return NextResponse.json({ error: 'User has already been invited to this group' }, { status: 409 });
    }
    ...
    return NextResponse.json({ invitation: { email: invitation.email, token: invitation.token } }, { status: 201 });
  } catch (error: any) {
